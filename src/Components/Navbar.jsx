...
const Navbar = () => {
  ...
  return (
    <Nav activeKey="/" className="fixed-top  nav justify-content-between col-lg-12 col-md-12 col-sm-12">
      <Nav.Item className="mx-5">
        <Row className="d-flex align-items-center">
          <Col xs="12" className="text-center">
            <Link className="d-flex align-items-center text-light text-decoration-none my-2">
              <img src={Logo} alt="TaskGenius Logo" height="50" className="mr-3" />
      ...
      {userToken && !isHomePage && (
        <Nav.Item className="mx-auto">
          <h1 className="text-light my-3 Nav-title">Welcome {capitalizeFirstLetter(userName)}</h1>
        </Nav.Item>
      )}

      {userToken && !isHomePage && (
        <Nav.Item className="mr-5 d-flex">
          <Button variant="danger" className="logout-button my-3 mr-2" onClick={handleLogout}>
            Logout
          </Button>
          ...
          (
            ...
            <Button variant="primary" className="new-task-button my-3" onClick={openModal}>
              New Task
            </Button>
          )}
